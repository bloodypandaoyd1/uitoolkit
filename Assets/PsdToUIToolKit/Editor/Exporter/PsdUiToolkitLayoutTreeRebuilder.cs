...
namespace PsdTools.UIToolKit
{
    internal static class PsdUiToolkitLayoutTreeRebuilder
    {
            ...
            bool inspectorMode)
        {
            ...
            int maxPasses = Math.Max(1, configMap.GetAutoLayoutConfig().maxNestingDepth);
        ...
        private static List<RebuildNodeState> RebuildContainedSiblings(List<RebuildNodeState> siblings, PsdUiToolkitLayerBounds rootBounds, PsdUiToolkitLayerConfigMap configMap)
        {
            ...
            for (int i = 0; i < siblings.Count; i++)
            {
                ...
                int nextIndex = i + 1;
                while (nextIndex < siblings.Count && ContainsBounds(host.Bounds, siblings[nextIndex].Bounds, configMap.GetAutoLayoutConfig().alignmentTolerance))
                {
        ...
        private static LayoutScore ScoreRow(List<RebuildNodeState> flowCandidates, PsdUiToolkitAutoLayoutGlobalConfig autoLayout)
        {
            if (flowCandidates.Count < 2)
        ...
        private static LayoutScore ScoreColumn(List<RebuildNodeState> flowCandidates, PsdUiToolkitAutoLayoutGlobalConfig autoLayout)
        {
            if (flowCandidates.Count < 2)
